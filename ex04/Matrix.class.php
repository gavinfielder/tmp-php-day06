...
class Matrix
{
	...
	{
		$r = new Vertex(array('x' => 0, 'y' => 0, 'z' => 0, 'w' => 0));
		$x = &$r->getRefX();
		$y = &$r->getRefY();
		$z = &$r->getRefZ();
		$w = &$r->getRefW();
		for ($i = 0; $i < 4; $i++)
		{
			$x += ($this->m(0,$i) * $v->getI($i));
			$y += ($this->m(1,$i) * $v->getI($i));
			$z += ($this->m(2,$i) * $v->getI($i));
			$w += ($this->m(3,$i) * $v->getI($i));
		}
		return $r;
	}
	...
	{
		$m = new Matrix(array('preset' => self::ZERO));
		for ($i = 0; $i < 4; $i++)
		{
			for ($j = 0; $j < 4; $j++)
			{
				$m->_m[$i][$j] = $this->_m[$j][$i];
			}
		}
		return $m;
	}
